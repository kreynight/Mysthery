...
export default function BatchGeneratorPage() {
  ...
  const createCollection = async (index: number) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(errData.error || "Failed to create collection.");
      }
    } catch {
          ...
          >
            ...
            {created[i] && (
              <div className="bg-green-50 border-t border-green-200 px-5 py-2.5 text-xs text-green-700">
                Collection created (inactive). Go to the collection editor to add images, set price, and activate.
              </div>
            )}
          </div>
        ))}
